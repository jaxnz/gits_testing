...
class CucumberCommand extends Command {
  ...
  @override
  String get description => 'Generate integration test from gherkin in .feature';
  ...
  void run() {
    ...
    final specificFeature = argResults?.rest.firstOrNull?.replaceAll('.feature', '').split(',');
    ...
    String pattern = specificFeature?.map((e) => '$e.feature').join('|') ?? '*.feature';
    ...
    FlutterHelper.start(
      'drive --driver=test_driver/integration_test.dart --target=integration_test/cucumber_test.dart -d chrome',
      progress: Progress((line) {
        ...
        if (line.contains('cucumber-report')) {
          ...
          if (which('npm').found) {
            ...
            print('🚀 Cucumber HTML report cucumber-report.html generated successfully 👍');
          }
